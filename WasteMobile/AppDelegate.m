...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions
{
   ...
   */
    ...
    //[PlotSampleGenerator TestingRandomess:1000 predictionPlot:25 measurePlot:8];
    //[PlotSampleGenerator TestingRandomess:1000 predictionPlot:30 measurePlot:10];
...
#pragma mark - code table initialization
- (void)initCodeTable{
...
-(void)patch120{
    [[AppPatch sharedInstance] patch120];
}
...
- (NSPersistentStoreCoordinator *)persistentStoreCoordinator
{
    ...
    NSURL *storeURL = [[self applicationDocumentsDirectory] URLByAppendingPathComponent:@"WasteDataModel.sqlite"];
    
    NSError *error = nil;
    _persistentStoreCoordinator = [[NSPersistentStoreCoordinator alloc] initWithManagedObjectModel:[self managedObjectModel]];
    // for lightweight migration in EForWaste
    ...
    if (![_persistentStoreCoordinator addPersistentStoreWithType:NSSQLiteStoreType configuration:nil URL:storeURL options:options error:&error]) {
         ...
         */
        NSLog(@"Error retrieving the PersistentStorCoordinator %@, %@", error, [error userInfo]);
    }
